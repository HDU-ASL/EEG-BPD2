...
class Solver(object):
    ...
    def train(self):
       ...
       # self.criterion2=criterion1=self.train_criterion1
        ...
        #eeg_best_tloss = float('inf') 
        eeg_best_tloss = float(200)  
        eeg_best_qloss = float('inf')  
        image_best_tloss = float(200)  
        ...
        for e in range(self.train_config.n_epoch):
            ...
            for batch in self.train_data_loader:
                ...
                #cls_loss =criterion(y_tilde_eeg, label) + criterion1(y_tilde_image, label) 
                #nn.CrossEntropyLoss
    ...
    def eval_eeg(self, mode=None, to_print=True, plot=True):
        ...
        if plot:
            ...
            # # ---- 2D 绘图 (x-y 平面) ----
            # fig1, ax1 = plt.subplots()
            # ax1.plot(gt_pose[:, ], gt_pose[:, 1], color='black', marker='o', linestyle='', label='Ground Truth Pose')
            ...
            realeg_pose = predeg_poses[:, :3]  
            gt_pose = targ_poses[:, :3]
            fig1, ax1 = plt.subplots()  
            ax1.plot(gt_pose[:, 1], gt_pose[:, 0], color='black', marker='o', linestyle='', label='Ground Truth Pose')  
            ax1.plot(realeg_pose[:, 1], realeg_pose[:, 0], color='red', marker='s', linestyle='', label='Predicted Pose (predeg_poses)')  
            for i in range(len(gt_pose)): 
                ax1.plot([gt_pose[i, 1], realeg_pose[i, 1]], [gt_pose[i, 0], realeg_pose[i, 0]], color='gray', linestyle='--', alpha=0.5)  
            ax1.set_xlabel('x [m]')  
            ax1.set_ylabel('y [m]')  
            ax1.legend()  
            ax1.grid(True)  
