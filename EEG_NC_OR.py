...
if __name__ == '__main__':
    ...
    if task=="desk01-T100-random":
        ...
        EEG_path="/home/hyx/实验五/Subject05/NP/sub05_random3_100.mat"
    ...
    if task=="OR-day":
        EEG_path = "/home/hyx/实验4/Day/实验4_new/原始数据/day.mat"
    ...
    if task=="OR-night":
        EEG_path = "/home/hyx/实验4/Day/实验4_new/原始数据/night.mat"
        eeg_data = EEG_reshape(EEG_path)
        sample_size=3700
    ...
    split_idx = int(0.7 * len(eeg_poses))  
    ...
    for e in range(3000):
        ...
        # real_pose = (predeg_poses[:, :3] )
        # gt_pose = (targ_poses[:, :3] ) 

        # # ========== XY 图 ==========
        # fig_xy, ax_xy = plt.subplots(figsize=(8, 6))
        # ax_xy.plot(gt_pose[:, 1], gt_pose[:, 0], 'ko', label='Ground Truth Pose')
        # ax_xy.plot(real_pose[:, 1], real_pose[:, 0], 'rx', label='Predicted Pose')

        # for i in range(len(gt_pose)):
        #     ax_xy.plot([gt_pose[i, 1], real_pose[i, 1]], [gt_pose[i, 0], real_pose[i, 0]], 'gray', linestyle='--', alpha=0.2)
