...
class Criterion(nn.Module):
    def __init__(self, t_loss_fn=nn.L1Loss(), q_loss_fn=nn.L1Loss(), sax=0.0, saq=0.0, learn_beta=False):
        ...
        self.sax = nn.Parameter(torch.tensor([sax], device='cuda:4'), requires_grad=learn_beta)  
        self.saq = nn.Parameter(torch.tensor([saq], device='cuda:4'), requires_grad=learn_beta)
